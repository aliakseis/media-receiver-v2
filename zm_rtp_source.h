...
private:
  ...
  //std::string mCname;    // Canonical name, for SDES
	...
	std::function<void(AVPacket&)> mCallback;
  ...
  uint32_t mRtpClock;
  uint32_t mRtpFactor;
  std::chrono::system_clock::time_point mBaseTimeReal;
  std::chrono::microseconds mBaseTimeNtp;
  uint32_t mBaseTimeRtp;

  //struct timeval mLastSrTimeReal;
  uint32_t mLastSrTimeNtpSecs;
  uint32_t mLastSrTimeNtpFrac;
  std::chrono::microseconds mLastSrTimeNtp;
...
private:
  ...
  void Capture(Buffer &buffer);

public:
  RtpSource( //int id, 
	  //const std::string &localHost, int localPortBase, const std::string &remoteHost, int remotePortBase, 
	  //uint32_t ssrc, 
	  std::function<void(AVPacket&)> callback,
	  _AVCODECID codecId,
	  uint16_t seq = 0, 
	  uint32_t rtpClock = 0, uint32_t rtpTime = 0 );
  ~RtpSource();
  ...
  void splitFrames(unsigned char* frame, size_t &frameSize);

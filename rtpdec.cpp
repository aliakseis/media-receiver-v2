...
{
    RTPDemuxContext *s;
    s = static_cast<RTPDemuxContext *>(av_mallocz(sizeof(RTPDemuxContext)));
    if (!s)
        return NULL;
                                     ...
                                     const uint8_t *buf, int len)
{
    ...
    if (s->parse_packet) {
        rv = s->parse_packet(s->ic, s->dynamic_protocol_context,
                                      nullptr, pkt, &timestamp, buf, len, seq,
                                      flags);
    } else {//if (st) {
...
static int enqueue_packet(RTPDemuxContext *s, uint8_t *buf, int len)
{
	...
	packet = static_cast<RTPPacket*>(av_mallocz(sizeof(*packet)));
    if (!packet)
                                ...
                                uint8_t **bufptr, int len)
{
    ...
    if (!buf) {
        ...
        if (s->parse_packet) {
            ...
            rv        = s->parse_packet(s->ic, s->dynamic_protocol_context,
                                                 nullptr, pkt, &timestamp, NULL, 0, 0,
                                                 flags);
            finalize_packet(s, pkt, timestamp);
            return rv;
        }
    ...
    //if (s->st) {
		...
		uint32_t arrival_ts = received * 90000 / 1000000; //av_rescale_q(received, { 1, AV_TIME_BASE },//AV_TIME_BASE_Q,
                              //             s->st->time_base);
        timestamp = AV_RB32(buf + 4);
...
//
//    if (!(value = static_cast<char*>(av_malloc(value_size)))) {
